  ...
  generatePageMarkUp = () => {
    const pageMarkUp1 = (
      <li className={(this.props.currentPage <= 1) ? 'page-item disabled' : 'page-item'}>
        <button className="page-link" onClick={this.props.handlePreviousPage}>Предыдущая</button>
      </li>
    );
    ...
    const pageMarkUp3 = (
      <li className={((this.props.currentPage * this.props.perPage) >= this.props.totalProductCount) ? 'page-item disabled' : 'page-item'}>
        <button className="page-link" onClick={this.props.handleNextPage}>Следующая</button>
      </li>
    );
